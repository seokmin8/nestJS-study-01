...
export class TypeOrmExModule {
  ...
  public static forCustomRepository<T extends new (...args: any[]) => any>(
    ...
    //받은 repo들을 for문을 돌면서 아까 setMetadata했으니까
    // 이번엔 get으로 얻어옵니다. Reflect는 반복 중 중간에 빼오는 느낌
    for (const repository of repositories) {
      ...
      // 프로세스 중 Factory function에 인수로 전달할 공급자 배열을 받고
      // Nest는 동일 순서로 주입 목록의 인스턴스를 인수로 Factory function에 전달.

      // providers(공급자들) 을 채워줘야 함
      providers.push({
